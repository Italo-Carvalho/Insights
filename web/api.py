...
class ApiService():
    ...
    def loginUser(self, username, password):
        login = requests.post(f"{self.url}/token/obtain/",
    ...
    def getData(self, username, password, page):
        ...
        if auth != 'denied':
            data =  requests.get(f"{self.url}/card/?page={page}"
    ...
    def createData(self, username, password, texto, tags):
        auth = self.loginUser(username, password)
        if auth != 'denied':
            return requests.post(f"{self.url}/card/"
    ...
    def delData(self , username, password, pk):
        auth = self.loginUser(username, password)
        if auth != 'denied':
            return requests.delete(f"{self.url}/card/{pk}"
                , headers={'Authorization': f'Bearer {auth}'}).status_code
        return 'denied'
    ...
    def putData(self, username, password, pk, texto, body):
        auth = self.loginUser(username, password)
        if auth != 'denied':
            return requests.put(f"{self.url}/card/{pk}"
                ,data={"texto": texto}
                , headers={'Authorization': f'Bearer {auth}'}).status_code
        return 'denied'